...
const WithdrawModal: React.FC<IProps> = (props) => {
    ...
    const [withdrawTime, setWithdrawTime] = useState(BigInt(0))
    ...
    useEffect(() => {
        const task = async () => {
            ...
            if (coreContracts && props.open) {
                ...
                const loanDetail = await coreContracts.processCenterContract._getOrderDetail(props.tradeId)
                setWithdrawTime(loanDetail.endTime)
                ...
                if (props.userState === 'lend' && props.loanState === 'PaidOff') {
                    ...
                    if (!withdrawn) {
                        ...
                        console.log('lend', tokenId, profit)
                        if (profit !== BigInt(0)) {
                            //
                            setWithdrawAmount(profit)
                ...
                if (Number(dayjs().unix()) < Number(dayjs.unix(Number(loanDetail.endTime)).add(10, 'hour').unix()))
                    setWithdrawButtonDisabled(true)
    ...
    return <Modal open={props.open}
        ...
        footer={
            ...
            <div className='flex justify-end' >
                ...
                <Button className={`h-40 text-16 w-1/2 ${!withdrawButtonDisabled ? 'primary-btn' : ''} ${props.userState === 'loan' ? 'w-1/2' : ''} `} type='primary'
                    loading={withdrawing}
    ...
    >
        <div className='mt-30 h-80 text-16'>
            <div>
            ...
            <div className='mt-10'>
                Withdraw Time: {dayjs.unix(Number(withdrawTime)).add(10, 'hour').format('YYYY-MM-DD HH:mm:ssZ')}
            </div>
        </div>
    </Modal >
}
